...
static int DemoNum = 10, PrimType = 0x0102, DisplayListID = -1, SpotTexID = -1, RandomDemo = 500000;
...
#define NLoops 4
	...
	int Group = pGenParticleGroups(1, NParticles);
	...
	pSize(1.0);
	pStartingAge(0);
	pColorD(PDSphere(pVec(0.7, 0.6, 0.8), 0.1));

	cerr << sizeof(pVec) << " " << NParticles;

	int AListID = pGenActionLists(1);

#if 1
	pNewActionList(AListID);

		pKillOld(300);

		pSource(NParticles, PDPoint(pVec(0, 10, 0)));

		pOrbitLine(pVec(0, 0, 1), pVec(1, 0, 0), 0.5, 0.1);
	...
	cerr << '\t' << Cl.Read() / (float)NLoops<< endl;
}
...
int main(int argc, char **argv)
{
	...
	glutAddMenuEntry("v: Toggle const color", 'v');
