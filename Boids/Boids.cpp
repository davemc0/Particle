...
void AvoidModel(Model& M)
{
    for (int i = 0; i < M.Objs.size(); i++) {
        ...
        for (int j = 0; j < Ob->verts.size(); j += 3) {
            ...
            P.Avoid(0.1, 1.0, 100,
                    PDTriangle(pVec(Ob->verts[j].x, Ob->verts[j].y, Ob->verts[j].z), pVec(Ob->verts[j + 1].x, Ob->verts[j + 1].y, Ob->verts[j + 1].z),
...
#define DAMPING 0.996
    ...
    if (BoidAvoid) { // Static collision avoidance.
...
#define NUM_FRAMES 30
    ...
    if (ShowText) {
        ...
        sprintf(msg, "%c%c%c%c%c%c%c", BoidGravity ? 'G' : ' ', BoidDamping ? 'D' : ' ', BoidFollowPoint ? 'F' : ' ', BoidCentering ? 'C' : ' ',
                BoidMatchVel ? 'M' : ' ', BoidNgbrCol ? 'N' : ' ', BoidAvoid ? 'A' : ' ');
...
void menu(int item)
{
    ...
    switch (item) {
    case 'G': BoidGravity = !BoidGravity; break;
    case 'D': BoidDamping = !BoidDamping; break;
    case 'F': BoidFollowPoint = !BoidFollowPoint; break;
    case 'C': BoidCentering = !BoidCentering; break;
    case 'M': BoidMatchVel = !BoidMatchVel; break;
    case 'N': BoidNgbrCol = !BoidNgbrCol; break;
    case 'A': BoidAvoid = !BoidAvoid; break;
    case 't': ShowText = !ShowText; break;
    case ' ': BoidPrep(); break;
    case 'c': DoMotion = !DoMotion; break;
    case 'v': ConstColor = !ConstColor; break;
    case 's':
        AntiAlias = !AntiAlias;
        if (AntiAlias) {
