...
PINLINEH void PContextActions_t::Avoid(Particle_t& m, const float magnitude, const float epsilon, const float look_ahead, const pDomain& dom)
{
    P_CHECK_ERR;
    switch (dom.Which) {
...
PINLINEH void PContextActions_t::Bounce(Particle_t& m, const float friction, const float resilience, const float fric_min_vel, const pDomain& dom)
{
    P_CHECK_ERR;
    switch (dom.Which) {
...
PINLINEH void PContextActions_t::CopyVertexB(Particle_t& m, const bool copy_pos, const bool copy_vel)
{
    P_CHECK_ERR;
    PACopyVertexB_Impl(m, PSh.get_dt(), copy_pos, copy_vel);
}

PINLINEH void PContextActions_t::Damping(Particle_t& m, const pVec& damping, const float min_vel, const float max_vel)
{
    P_CHECK_ERR;
    PADamping_Impl(m, PSh.get_dt(), damping, min_vel, max_vel);
}

PINLINEH void PContextActions_t::RotDamping(Particle_t& m, const pVec& damping, const float min_vel, const float max_vel)
{
    P_CHECK_ERR;
    PARotDamping_Impl(m, PSh.get_dt(), damping, min_vel, max_vel);
}

PINLINEH void PContextActions_t::Explosion(Particle_t& m, const pVec& center, const float radius, const float magnitude, const float stdev, const float epsilon)
{
    P_CHECK_ERR;
    PAExplosion_Impl(m, PSh.get_dt(), center, radius, magnitude, stdev, epsilon);
}

PINLINEH void PContextActions_t::Gravity(Particle_t& m, const pVec& dir)
{
    P_CHECK_ERR;
    PAGravity_Impl(m, PSh.get_dt(), dir);
}

PINLINEH void PContextActions_t::Jet(Particle_t& m, const pDomain& dom, const pDomain& accel)
{
    P_CHECK_ERR;
    PAJet_Impl(m, PSh.get_dt(), dom, accel);
}

PINLINEH void PContextActions_t::Move(Particle_t& m, const bool move_velocity, const bool move_rotational_velocity)
{
    P_CHECK_ERR;
    PAMove_Impl(m, PSh.get_dt(), move_velocity, move_rotational_velocity);
}

PINLINEH void PContextActions_t::OrbitLine(Particle_t& m, const pVec& p, const pVec& axis, const float magnitude, const float epsilon, const float max_radius)
{
    P_CHECK_ERR;
    PAOrbitLine_Impl(m, PSh.get_dt(), p, axis, magnitude, epsilon, max_radius);
}

PINLINEH void PContextActions_t::OrbitPoint(Particle_t& m, const pVec& center, const float magnitude, const float epsilon, const float max_radius)
{
    P_CHECK_ERR;
    PAOrbitPoint_Impl(m, PSh.get_dt(), center, magnitude, epsilon, max_radius);
}

PINLINEH void PContextActions_t::RandomAccel(Particle_t& m, const pDomain& gen_acc)
{
    P_CHECK_ERR;
    PARandomAccel_Impl(m, PSh.get_dt(), gen_acc);
}

PINLINEH void PContextActions_t::RandomDisplace(Particle_t& m, const pDomain& gen_disp)
{
    P_CHECK_ERR;
    PARandomDisplace_Impl(m, PSh.get_dt(), gen_disp);
}

PINLINEH void PContextActions_t::RandomVelocity(Particle_t& m, const pDomain& gen_vel)
{
    P_CHECK_ERR;
    PARandomVelocity_Impl(m, PSh.get_dt(), gen_vel);
}

PINLINEH void PContextActions_t::RandomRotVelocity(Particle_t& m, const pDomain& gen_vel)
{
    P_CHECK_ERR;
    PARandomRotVelocity_Impl(m, PSh.get_dt(), gen_vel);
}

PINLINEH void PContextActions_t::Restore(Particle_t& m, const float time_left, const bool vel, const bool rvel)
{
    P_CHECK_ERR;
    PARestore_Impl(m, PSh.get_dt(), time_left, vel, rvel);
}

PINLINEH void PContextActions_t::SpeedClamp(Particle_t& m, const float min_speed, const float max_speed)
{
    P_CHECK_ERR;
    PASpeedClamp_Impl(m, PSh.get_dt(), min_speed, max_speed);
}

PINLINEH void PContextActions_t::TargetColor(Particle_t& m, const pVec& color, const float alpha, const float scale)
{
    P_CHECK_ERR;
    PATargetColor_Impl(m, PSh.get_dt(), color, alpha, scale);
}

PINLINEH void PContextActions_t::TargetSize(Particle_t& m, const pVec& size, const pVec& scale)
{
    P_CHECK_ERR;
    PATargetSize_Impl(m, PSh.get_dt(), size, scale);
}

PINLINEH void PContextActions_t::TargetVelocity(Particle_t& m, const pVec& vel, const float scale)
{
    P_CHECK_ERR;
    PATargetVelocity_Impl(m, PSh.get_dt(), vel, scale);
}

PINLINEH void PContextActions_t::TargetRotVelocity(Particle_t& m, const pVec& rot_velocity, const float scale)
{
    P_CHECK_ERR;
    PATargetRotVelocity_Impl(m, PSh.get_dt(), rot_velocity, scale);
}

PINLINEH void PContextActions_t::Vortex(Particle_t& m, const pVec& tip, const pVec& axis, const float tightnessExponent, const float max_radius,
...
PINLINEH void PContextActions_t::Follow(Particle_t& m, const float magnitude, const float epsilon, const float max_radius)
{
    P_CHECK_ERR;
    PAFollow_Impl(m, PSh.get_dt(), magnitude, epsilon, max_radius, PSh.get_const_pgroup_begin(), PSh.get_const_pgroup_end());
}

PINLINEH void PContextActions_t::Gravitate(Particle_t& m, const float magnitude, const float epsilon, const float max_radius)
{
    P_CHECK_ERR;
    PAGravitate_Impl(m, PSh.get_dt(), magnitude, epsilon, max_radius, PSh.get_const_pgroup_begin(), PSh.get_const_pgroup_end());
}

PINLINEH void PContextActions_t::MatchVelocity(Particle_t& m, const float magnitude, const float epsilon, const float max_radius)
{
    P_CHECK_ERR;
    PAMatchVelocity_Impl(m, PSh.get_dt(), magnitude, epsilon, max_radius, PSh.get_const_pgroup_begin(), PSh.get_const_pgroup_end());
}

PINLINEH void PContextActions_t::MatchRotVelocity(Particle_t& m, const float magnitude, const float epsilon, const float max_radius)
{
    P_CHECK_ERR;
...
PINLINEH void PContextActions_t::Callback(Particle_t& m, P_PARTICLE_CALLBACK_ACTION callbackFunc, const pdata_t call_data)
{
    P_CHECK_ERR;
    (*callbackFunc)(m, call_data, PSh.get_dt());
}

PINLINEH void PContextActions_t::KillOld(Particle_t& m, const float age_limit, const bool kill_less_than)
{
    P_CHECK_ERR;
    PAKillOld_Impl(m, PSh.get_dt(), age_limit, kill_less_than);
}

PINLINEH void PContextActions_t::Sink(Particle_t& m, const bool kill_inside, const pDomain& kill_pos_dom)
{
    P_CHECK_ERR;
    PASink_Impl(m, PSh.get_dt(), kill_inside, kill_pos_dom);
}

PINLINEH void PContextActions_t::SinkVelocity(Particle_t& m, const bool kill_inside, const pDomain& kill_vel_dom)
{
    P_CHECK_ERR;
