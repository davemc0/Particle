...
public:
    ...
    size_t GetParticlePointer(float*& ptr,       ///< the returned pointer to the particle data
                              ...
                              size_t& data1Ofs   ///< the number of floats from returned ptr to the first particle's data parameter, which is a 64-bit integer,
                                                 ///< not a float
    );
...
public:
    ...
    void Avoid(float magnitude,        ///< how drastically the particle velocities are modified to avoid the obstacle at each time step.
               const float epsilon,    ///< The amount of acceleration falls off inversely with the squared distance to the edge of the domain. But when that
                                       ///< distance is small, the acceleration would be infinite, so epsilon is always added to the distance.
    ...
    void Callback(P_PARTICLE_CALLBACK_ACTION callbackFunc, ///< Pointer to function of yours to call. Can be NULL if using only CUDA execution.
                  const std::string& callbackStr, ///< The source code of the callback function, which is emitted as part of the kernel. Can be an empty string
                                                  ///< if not using CUDA execution.
    ...
    void Explosion(const pVec& center,         ///< center point of shock wave
                   ...
                   const float epsilon = P_EPS ///< The amount of acceleration falls off inversely with the squared distance to the edge of the domain. But when
                                               ///< that distance is small, the acceleration would be infinite, so epsilon is always added to the distance.
    );
    ...
    void Follow(float magnitude = 1.0f,      ///< scales each particle's acceleration
                const float epsilon = P_EPS, ///< The amount of acceleration falls off inversely with the squared distance to the edge of the domain. But when
                                             ///< that distance is small, the acceleration would be infinite, so epsilon is always added to the distance.
                const float max_radius = P_MAXFLOAT ///< defines the sphere of influence of this action. No particle further than max_radius from its
    ...
    void Gravitate(const float magnitude = 1.0f, ///< scales each particle's acceleration
                   const float epsilon = P_EPS, ///< The amount of acceleration falls off inversely with the squared distance to the edge of the domain. But when
                                                ///< that distance is small, the acceleration would be infinite, so epsilon is always added to the distance.
                   const float max_radius = P_MAXFLOAT ///< defines the sphere of influence of this action. No particle further than max_radius from another
    ...
    void
    MatchVelocity(const float magnitude = 1.0f, ///< scales each particle's acceleration
                  const float epsilon = P_EPS, ///< The amount of acceleration falls off inversely with the squared distance to the edge of the domain. But when
                                               ///< that distance is small, the acceleration would be infinite, so epsilon is always added to the distance.
                  const float max_radius = P_MAXFLOAT ///< defines the sphere of influence of this action. No particle further than max_radius from another
                                                      ///< particle is affected.
    ...
    void MatchRotVelocity(const float magnitude = 1.0f, ///< scales each particle's acceleration
                          const float epsilon = P_EPS,  ///< The amount of acceleration falls off inversely with the squared distance to the edge of the domain.
                                                        ///< But when that distance is small, the acceleration would be infinite, so epsilon is always added to
                                                        ///< the distance.
                          const float max_radius = P_MAXFLOAT ///< defines the sphere of influence of this action. No particle further than max_radius from
                                                              ///< another particle is affected.
    );
    ...
    void OrbitLine(const pVec& p,                ///< a point on the line
                   ...
                   const float epsilon = P_EPS, ///< The amount of acceleration falls off inversely with the squared distance to the edge of the domain. But when
                                                ///< that distance is small, the acceleration would be infinite, so epsilon is always added to the distance.
                   const float max_radius = P_MAXFLOAT ///< defines the cylinder of influence of this action. No particle further than max_radius from the line
                                                       ///< is affected.
    );
    ...
    void OrbitPoint(const pVec& center,           ///< accelerate toward this point
                    ...
                    const float epsilon = P_EPS, ///< The amount of acceleration falls off inversely with the squared distance to the edge of the domain. But when
                                                 ///< that distance is small, the acceleration would be infinite, so epsilon is always added to the distance.
                    const float max_radius = P_MAXFLOAT ///< defines the sphere of influence of this action. No particle further than max_radius from the center
                                                        ///< is affected.
    );
    ...
    void Sort(const pVec& eye, const pVec& look,
              const bool front_to_back = false, ///< true to sort in front-to-back order instead of back-to-front
              const bool clamp_negative = false ///< true to set negative dot product values to zero before sorting. This speeds up sorting time. Particles
                                                ///< behind the viewer won't be visible so their relative order doesn't matter.
    );
    ...
    void TargetRotVelocity(const pVec& rvel, ///< rotational velocity
                           const float scale ///< what percent of the way from the current rotational velocity to the target rotational velocity to transition
                                             ///< in unit time
    );
    ...
    void
    Vortex(const pVec& center,            ///< tip of the vortex
           ...
           const float max_radius, ///< defines the radius at the top of the vortex and the infinite cylinder of influence. No particle further than max_radius
                                   ///< from the axis is affected.
...
class ParticleContext_t : public PContextActionList_t, public PContextParticleGroup_t, public PContextActions_t {
public:
