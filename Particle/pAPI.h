...
/// This class contains the Action API calls for the older, less performant, but perhaps more elegant API.
///
...
public:
    ...
    void Bounce(float friction,           ///< tangential component of the outgoing velocity vector is scaled by (1 - friction)
    ...
    void Callback(P_PARTICLE_CALLBACK_ACTION callbackFunc, ///< Pointer to function of yours to call.
    ...
    void CopyVertexB(const bool copy_pos = true, ///< If true, sets the PositionB of each particle to the current position of that particle. This makes each
                                                 ///< particle remember this position so it can later return to it using the Restore() action.
                     const bool copy_vel = false ///< If true, sets the velocityB of each particle to the current velocity of that particle. This can be useful
                                                 ///< for computing the orientation of the particle by copying a particle's velocity at the beginning of each
                                                 ///< time step. Then when drawing a particle, the cross-product velocity and velocityB yields a tangent vector.
    );
    ...
    void RandomAccel(const pDomain& dom ///< choose acceleration from this domain
    );
    ...
    void RandomDisplace(const pDomain& dom ///< choose position offset from this domain
    );
    ...
    void RandomVelocity(const pDomain& dom ///< choose velocity from this domain
    );
    ...
    /// This function is not affected by dt.
    void RandomRotVelocity(const pDomain& dom ///< choose rotational velocity from this domain
    );
    ...
    void SpeedClamp(const float min_speed, ///< set velocity vectors below min_speed to min_speed
...
class ParticleContext_t : public PContextInlineActions_t, public PContextActionList_t, public PContextParticleGroup_t, public PContextActions_t {
public:
