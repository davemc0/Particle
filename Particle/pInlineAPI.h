...
#ifndef particle_inline_api_h
#define particle_inline_api_h
...
public:
    ...
    void IAvoid(float magnitude,        ///< how drastically the particle velocities are modified to avoid the obstacle at each time step.
    ...
    void IBounce(float friction,           ///< tangential component of the outgoing velocity vector is scaled by (1 - friction)
    ...
    void ICallback(P_PARTICLE_CALLBACK_ACTION callbackFunc, ///< Pointer to function of yours to call.
    ...
    void ICopyVertexB(const bool copy_pos = true, ///< If true, sets the PositionB of each particle to the current position of that particle. This makes each
    ...
    void IDamping(const pVec& damping,             ///< component-wise multiply this vector by the velocity vector
    ...
    ///
    /// There are no bounds on the damping constants. Thus, by giving values greater than 1.0 they may be used to speed up particles instead of slow them down.
    void IRotDamping(const pVec& damping,             ///< component-wise multiply this vector by the rotational velocity vector
                     const float min_vel = 0.0f,      ///< only dampen if velocity magnitude is greater than min_vel
                     const float max_vel = P_MAXFLOAT ///< only dampen if velocity magnitude is less than max_vel
    );
    ...
    void IExplosion(const pVec& center,         ///< center point of shock wave
    ...
    void IFollow(float magnitude = 1.0f,             ///< scales each particle's acceleration
                 const float epsilon = P_EPS,        ///< added to distance to dampen acceleration
    ...
    void IGravitate(const float magnitude = 1.0f,       ///< scales each particle's acceleration
                    const float epsilon = P_EPS,        ///< added to distance to dampen acceleration
    ...
    void IJet(const pDomain& dom,  ///< apply jet to particles in this domain
    ...
    void IKillOld(const float age_limit,
    ...
    void IMatchVelocity(const float magnitude = 1.0f,       ///< scales each particle's acceleration
                        const float epsilon = P_EPS,        ///< added to distance to dampen acceleration
                        const float max_radius = P_MAXFLOAT ///< no particle further than max_radius from another particle is affected
    );
    ...
    ///
    /// Using an epsilon similar in size to magnitude can increase the range of influence of nearby particles on this particle.
    void IMatchRotVelocity(const float magnitude = 1.0f,       ///< scales each particle's acceleration
                           const float epsilon = P_EPS,        ///< added to distance to dampen acceleration
                           const float max_radius = P_MAXFLOAT ///< no particle further than max_radius from another particle is affected
    );
    ...
    void IMove(const bool move_velocity = true,           ///< apply velocity to position.
    ...
    void IOrbitLine(const pVec& p,                      ///< a point on the line
    ...
    void IOrbitPoint(const pVec& center,                 ///< accelerate toward this point
                     const float magnitude = 1.0f,       ///< scales each particle's acceleration
                     const float epsilon = P_EPS,        ///< added to distance to dampen acceleration
    ...
    void IRandomAccel(const pDomain& dom ///< choose acceleration from this domain
    );
    ...
    void IRandomDisplace(const pDomain& dom ///< choose position offset from this domain
    );
    ...
    void IRandomVelocity(const pDomain& dom ///< choose velocity from this domain
    );
    ...
    /// This function is not affected by dt.
    void IRandomRotVelocity(const pDomain& dom ///< choose rotational velocity from this domain
    );
    ...
    void IRestore(const float time,      ///< how long more until particles should arrive at target position and orientation
    ...
    void ISink(const bool kill_inside, ///< true to kill particles inside the domain
    ...
    void ISinkVelocity(const bool kill_inside, ///< true to kill particles with velocities inside the domain
    ...
    void ISort(const pVec& eye,                  ///< eye point is a point on the line the particles project onto
    ...
    void ISource(const float particle_rate, ///< how many particles to add per unit time
    ...
    void ISpeedClamp(const float min_speed, ///< set velocity vectors below min_speed to min_speed
    ...
    void ITargetColor(const pVec& color, ///< target color
    ...
    ///
    /// The value of scale will usually be very small (less than 0.01) to yield a gradual transition.
    void ITargetSize(const pVec& size, ///< target size
    ...
    /// The value of scale will usually be very small (less than 0.01) to yield a gradual transition.
    void ITargetVelocity(const pVec& vel,  ///< target velocity
    ...
    /// The value of scale will usually be very small (less than 0.01) to yield a gradual transition.
    void ITargetRotVelocity(const pVec& rvel, ///< rotational velocity
    ...
    void IVertex(const pVec& v,             ///< position of particle to create
                 const pSourceState& SrcSt, ///< all other particle attributes are chosen from this source state
    ...
    void IVortex(const pVec& center,            ///< tip of the vortex
...
protected:
    ...
    void IInternalSetup(std::shared_ptr<PInternalState_t> Sr); // Calls this after construction to set up the PS pointer
};
}; // namespace PAPI
